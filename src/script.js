...
const ambientLight1 = new THREE.AmbientLight(0xffffff, 0.7)
 ...
 * Models
 */
gltfLoader.load(`/models/toilet_2.glb`, (gltf) => {
  toilet[0] = gltf.scene
  ...
  animations[0].play = mixer[0].clipAction(
    THREE.AnimationClip.findByName(gltf.animations, "Variation_2")
  )
...
const animate = () => {
  ...
  if (waterFlow[0]) {
    waterFlow[0].map.offset.x += 0.007
    waterFlow[0].map.offset.y -= 0.007
  }

  if (mixer[1]) mixer[1].update(deltaTime)
  if (waterFlow[1]) {
    waterFlow[1].map.offset.x += 0.007
    waterFlow[1].map.offset.y -= 0.007
  }
